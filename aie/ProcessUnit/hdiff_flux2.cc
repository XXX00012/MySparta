...
void hdiff_flux2(input_window_int32* flux_inter1_win,
  ...
  alignas(32) int32_t out_buf[COL];

  for (int i = 0; i < 2 * COL; ++i) flux_inter1_buf[i] = window_readincr(flux_inter1_win);
  for (int i = 0; i < 2 * COL; ++i) flux_inter2_buf[i] = window_readincr(flux_inter2_win);
  for (int i = 0; i < 2 * COL; ++i) flux_inter3_buf[i] = window_readincr(flux_inter3_win);
  for (int i = 0; i < 2 * COL; ++i) flux_inter4_buf[i] = window_readincr(flux_inter4_win);
  for (int i = 0; i < 2 * COL; ++i) flux_inter5_buf[i] = window_readincr(flux_inter5_win);
  ...
  int32_t *restrict out_base = out_buf;
  ...
  v8int32 *restrict ptr_forward = (v8int32 *)flux_inter1_base;
  v8int32 *ptr_out = (v8int32 *)out_base;
  ...
  for (unsigned i = 0; i < COL / 8; i++)
    chess_prepare_for_pipelining chess_loop_range(1, ) {
      ...
      ptr_forward = (v8int32 *)flux_inter1_base + 2 * i;
      ...
      ptr_forward = (v8int32 *)flux_inter2_base + 2 * i;
      flux_sub = *ptr_forward++;
      flux_interm_sub = *ptr_forward;
      ...
      ptr_forward = (v8int32 *)flux_inter3_base + 2 * i;
      flux_sub = *ptr_forward++;
      flux_interm_sub = *ptr_forward;
      ...
      ptr_forward = (v8int32 *)flux_inter4_base + 2 * i;
      flux_sub = *ptr_forward++;
      flux_interm_sub = *ptr_forward;
      ...
      ptr_forward = (v8int32 *)flux_inter5_base + 2 * i;
  ...
  for (int i = 0; i < COL; ++i) window_writeincr(out_win, out_buf[i]);
}